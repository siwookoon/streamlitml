...
def main():
    ...
    with my_grid[0][1]:
        ...
        for i in range(0, 10):
            with st.spinner('Wait for it... **%s seconds**' % (10-i)):
                time.sleep(1)
        st.success('Done!')
    ...
    with my_grid[1][0]:
        ...
        st.write('url : https://docs.streamlit.io/library/api-reference/status/st.snowflakes')
        st.snow()
    ...
    with my_grid[1][2]:
        st.header('Exception')
        st.write('url : https://docs.streamlit.io/library/api-reference/status/st.excepttion')
        a = 1
        b = 0
        try:
            a/b
